...
인증키 발급만으로 모든 KRX 데이터 상품이 자동 활성화되지는 않습니다. KRX
`마이페이지 → API 이용현황`에서 실제로 사용할 시장의 **종목기본정보**와
**일별매매정보**가 `승인`인지 확인해야 합니다. KOSPI만 조사한다면 유가증권
시장 두 서비스를, KOSDAQ·KONEX까지 조사한다면 각 시장의 같은 두 서비스를
추가로 승인합니다. 승인 뒤 패널의 KRX 배지를 클릭하면 공식 API 최소 실조회를
다시 수행하며, 성공하면 초록색 `MCP 1.0.0` 상태가 표시됩니다.
...
실운영 검증에서는 승인 완료 후 KOSPI 종목기본정보·일별매매정보 공식 API와
두 MCP 도구가 모두 HTTP 200으로 응답했고, Obsidian KRX 탭에서도 동일 기준일의
종가·거래량·시가총액을 반환하는 것을 확인했습니다.
