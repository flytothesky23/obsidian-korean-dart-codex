...
4. `korea-stock-mcp@1.4.1`은 `KRX_API_KEY` 이름만 전달하고
   `enabled_tools=["get_stock_base_info","get_stock_trade_info"]`로 제한한다.
5. 직접 MCP initialize/tools/list와 Codex app-server의
   `mcpServerStatus/list` 도구 인벤토리를 각각 검증한다. 기존 Codex 설정
   모드를 선택한 경우에만 `codex mcp get korean-dart --json`도 확인한다.
6. 저장소를 빌드하고 `main.js`, `manifest.json`, `styles.css`를
   `<vault>/.obsidian/plugins/korean-dart-codex`에 배포한다.
7. Obsidian에서 플러그인을 활성화하고 DART/KRX 탭, 각 MCP 상태 배지,
   app-server 스트리밍, KRX 기준일 frontmatter를 확인한다.
8. 비밀값을 출력하거나 소스/커밋에 남기지 않는다.
```
...
key fields, the two-tool KRX allowlist, direct MCP initialize/tools/list, Codex
app-server MCP inventory, plugin build artifacts, DART/KRX tabs, streaming,
health badges, and saved-note source/date metadata. Do not expose credentials or
place key values in CLI arguments.
```
